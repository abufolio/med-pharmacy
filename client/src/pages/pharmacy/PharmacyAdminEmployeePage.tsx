...
export function PharmacyAdminEmployeePage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['pharmacy-employees', page],
    queryFn: () => employeeApi.list({ page, limit: 20 }),
  });

  const createMutation = useMutation({
    mutationFn: (data: any) => employeeApi.create({ ...data, pharmacyId: user?.pharmacyId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['pharmacy-employees'] });
      setCreateOpen(false);
