...
export function EmployeeCustomerPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['employee-customers', page, search],
            ...
            <TableBody>
              {users.map((u: any) => (
                <TableRow key={u.id}>
                  ...
                  <TableCell>{u.language?.toUpperCase()}</TableCell>
                  <TableCell><Badge variant={getStatusColor(u.status) as any}>{getStatusLabel(u.status)}</Badge></TableCell>
                </TableRow>
              ))}
              {users.length === 0 && (
