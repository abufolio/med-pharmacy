...
exports.MESSAGES = exports.BOT_COMMANDS = void 0;
...
exports.MESSAGES = {
    welcome: (firstName) => `Assalomu alaykum, <b>${firstName}</b>! 👋\n\n` +
    ...
    balance: (balance) => `💰 <b>Wallet balansingiz:</b> ${balance.toLocaleString()} so'm`,
    noWallet: '💰 Sizda wallet hali yaratilmagan.',
    cashbackList: (cashbacks) => {
        if (cashbacks.length === 0)
            return '🎁 Keshbeklar mavjud emas.';
        const lines = cashbacks.map((c, i) => `${i + 1}. ${c.amount.toLocaleString()} so'm — ${c.status} (${c.date})`);
    ...
    cardLinked: (phone) => `✅ Telefon raqamingiz <b>${phone}</b> muvaffaqiyatli bog'landi!`,
    ...
    notifications: (notifications) => {
        if (notifications.length === 0)
            return '🔔 Sizda yangi xabarlar yo\'q.';
    ...
    stats: (data) => `📊 <b>Bot statistikasi:</b>\n\n` +
    ...
    broadcastSent: (count) => `✅ Xabar ${count} ta foydalanuvchiga yuborildi.`,
