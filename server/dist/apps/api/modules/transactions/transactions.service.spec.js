...
describe('TransactionsService', () => {
    ...
    let prisma;
    let audit;
    function mockTransactionCallback(txMock) {
        prisma.client.$transaction.mockImplementation(async (cb) => cb(txMock));
    }
    beforeEach(async () => {
        const txMock = {
    ...
    describe('findById', () => {
        ...
        it('should throw NotFoundException if not found', async () => {
            ...
            await expect(service.findById('tx-invalid')).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('reverseTransaction', () => {
        it('should reverse a completed transaction with cashback', async () => {
            ...
            expect(txMock.transaction.update).toHaveBeenCalledWith(expect.objectContaining({ data: { status: 'REVERSED' } }));
            expect(txMock.cashback.update).toHaveBeenCalledWith(expect.objectContaining({ data: { status: 'ROLLED_BACK' } }));
        ...
        it('should throw NotFoundException if transaction not found', async () => {
            prisma.client.transaction.findUnique.mockResolvedValue(null);
            await expect(service.reverseTransaction('tx-invalid')).rejects.toThrow(common_1.NotFoundException);
        });
        it('should throw BadRequestException if already reversed', async () => {
            ...
            await expect(service.reverseTransaction('tx-1')).rejects.toThrow(common_1.BadRequestException);
        });
    });
});
